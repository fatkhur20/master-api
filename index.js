...
async function handleMasterRequest(request, env, ctx) {
    ...
    // We can add a manual trigger endpoint for debugging.
    const url = new URL(request.url);
    if (request.method === 'POST' && url.pathname === '/force-health') {
        ctx.waitUntil(triggerHealthCheck(env, ctx));
        return jsonResponse({ ok: true, message: 'Health check process manually triggered.' });
    }
    return textResponse('Master worker is active. Awaiting cron triggers.', 200);
}
...
async function triggerHealthCheck(env, ctx) {
    ...
    try {
        ...
        // Simple round-robin dispatch of single proxies.
        const dispatchPromises = proxies.map((proxy, idx) => {
            ...
            return fetch(`${endpoint.replace(/\/$/, '')}/check-batch`, {
                ...
                body: JSON.stringify({ batch: [proxy], job_id: job.job_id })
            });
        });

        await Promise.all(dispatchPromises);
        console.log(`Dispatched ${proxies.length} individual proxy checks for job ${job.job_id}.`);
...
async function handleSupportRequest(request, env, ctx) {
  ...
  if (request.method === 'POST' && path === '/check-batch') {
    ...
    // In this simpler model, the support worker posts results directly to the backend API.
    ctx.waitUntil(postResultsToApi(results, payload.job_id, env));
...
async function postResultsToApi(results, job_id, env) {
  if (!env.API_RESULT_URL) {
  ...
  try {
    ...
    await fetch(env.API_RESULT_URL, {
      ...
      body: JSON.stringify({ job_id, results })
    });
  } catch (e) {
